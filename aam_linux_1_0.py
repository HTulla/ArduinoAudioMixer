...
def printMenu():
    ...
    print("3) Print available serial ports")
    print("4) Reload settings file")
    print("0) Exit")
...
def parse_kbd_input(input_str):
    print()
    ## Exit
    if input_str == "0":
        print("Exiting...")
        sys.exit(0)
    ## Running audioprocesses
    elif input_str == "1":
        pulse = pulsectl.Pulse()
        print('\nRunning audioprocesses:')
        #print(get_audioprocesses_names(get_audioprocesses(pulse)) + 'asd')
    ...
    ## Available com ports
        ...
        for port in serial_ports:
            print(port)
    ## Reload settings filename
    elif input_str == "4":
        print("Reloading settings from '{}' ...".format(settingsfile))
...
def get_audioprocesses_names(all_prosesses):
    ...
    try:
        for process in all_prosesses:
            ...
            if process.proplist.get('application.process.binary') is not None:
                process_names.append(process.proplist.get('application.process.binary'))
            else:
                process_names.append(process.proplist.get('application.name'))
    ...
    try:
        ...
        for program_names in program_name_list:
            ...
            for program_name in program_names:
                ...
                #print(current_processes_names)
                for process_name in current_processes_names:
                    #print(process_name
                    # Handle the none thrown by audio loopback
                    if process_name == None:
                        pass
                    elif program_name.lower() == process_name.lower():
...
def main():
    try:
        ...
        if not arduino_serial:
            ...
            for i, port in enumerate(available_serial_ports):
                print("{}) {}".format(i, port))
            portnumber = int(input("Select port from above ports by number (0 - {}): ".format(i)))
            try:
                #arduino_serial = connect_serial(port)
                arduino_serial = connect_serial(available_serial_ports[portnumber])
                if not arduino_serial:
                    exit(0)
            except:
                exit(0)
        ...
        ## Input queue for keyboard input
        inputQueue = queue.Queue()

        # Create and start a thread to read keyboard inputs. Set daemon=True to autokill thread when main tread exits
        ...
        while True:
            ...
            if (inputQueue.qsize() > 0):
                ...
                #print(input_str)
                ## Parse input
                parse_kbd_input(input_str)
                ## Print menu
                printMenu()

            ## If new data is received on previous loop, set new 'old_volume_data'
            if volume_data:
